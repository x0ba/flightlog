export type DotStyle = Record<string, string | number | undefined>;

export function dotStyle(style: DotStyle): string {
	return Object.entries(style)
		...
		.map(([key, value]) => {
			...
			const needsPx =
				typeof value === 'number' &&
				!key.startsWith('--') &&
				!['opacity', 'z-index', 'font-weight', 'line-height', 'flex-grow'].includes(prop);
			const rendered = needsPx ? `${value}px` : String(value);
